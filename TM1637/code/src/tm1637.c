...
static uint8_t _colomn = 0;

#define MICRO_DELAY(x) (delay(1))
...
void tm1637_setCLK(uint8_t state);
void tm1637_setDIO(uint8_t state);
uint8_t tm1637_getDIO(void);
...
{
#ifdef __USE_HAL
  return (uint8_t)HAL_GPIO_ReadPin(cdPort, dataPin);
#else
  return (uint8_t)(cdPort->IDR & dataPin);
#endif
}
//Start sequence
void tm1637_TWI_start(void)
